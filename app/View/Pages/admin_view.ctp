...
<table class="zebra-striped">
	...
	<tr class="<?php echo $comment['active'] ? 'active' : 'inactive' ?>">
		...
		<td>
			<?php if (!empty($comment['link'])): ?><a href="<?php echo $comment['link'] ?>" target="_blank"><?php echo $comment['user']; ?></a>
			<?php else: echo $comment['user']; endif; ?>
		</td>
		...
		<?php echo $this->Html->link(__('Supprimer'), array('controller' => 'comments', 'action' => 'delete', $comment['id']), null, __('Are you sure you want to delete # %s?', $comment['id'])); ?>
		</td>
	</tr>
	<?php endforeach; ?>
</table>